...
public struct PropertyChange {
    ...
    let name: String
    /// The value of the property before the change occurred. This is not
    /// supplied if the change happened on the same thread as the notification
    /// or for List properties.
    let oldValue: Any?
    /// The value of the property after the change occurred. This is not
    /// supplied for List properties and will always be nil.
    let newValue: Any?
}
...
public enum ObjectChange {
     ...
     */
    case change(changes: [PropertyChange])
...
extension Object {
     ...
     */
    func addNotificationBlock(_ block: @escaping (ObjectChange) -> Void) -> NotificationToken {
        return RLMAddNotificationBlock(self, block)
    }
}
